...
fn update(series_id: i32, blob_id: i32, form: InfoBlobForm, conn: PooledConn) -> Result<InfoBlob> {
    InfoBlob::update(&*conn, series_id, blob_id, form)
}

fn new(series_id: i32, form: InfoBlobForm, conn: PooledConn) -> Result<InfoBlob> {
...
pub fn routes(db_filter: PooledConnFilter) -> BoxedFilter<(impl Reply,)> {
    ...
    let select_types = warp::filters::method::get2()
        .and(path!("info" / i32 / "types" / String))
        .and(path::end())
        .and(db_filter.clone())
        .map(select_types)
        .map(api_response);
    ...
    warp::any()
        .and(all.or(new).or(update).or(select).or(delete).or(select_types))
